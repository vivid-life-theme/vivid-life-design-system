...
# LOCAL ADDITION (vivid-life-design-system only, not in the canonical plugin
# copy): handoff/ — port-distribution artifacts read by downstream port repos.
# The canonical script has no generic concept of this directory; re-add it
# here on every /auditing-config refresh so refreshing to a newer canonical
# version doesn't silently drop handoff/README.md and handoff/SKILL.md from
# the table again.
