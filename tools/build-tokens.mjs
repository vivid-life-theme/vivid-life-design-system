...
export function relLum(hex) {
  const [r, g, b] = parseHex(hex).slice(0, 3).map(v => v / 255);
...
export function hoverOver({ flavor, bg } = {}) {
  ...
  return flavor === 'dark' ? alphaOver('#ffffff', bg, 0.08)
...
export function activeOver({ flavor, bg } = {}) {
  ...
  return flavor === 'dark' ? alphaOver('#ffffff', bg, 0.12)
...
export function json5ToJson(src) {
  ...
  out = out.replace(/([{,]\s*)([A-Za-z_$][A-Za-z0-9_$]*|[0-9]+)\s*:/g,
                    (m, pre, key) => `${pre}"${key}":`);
  return out;
}
...
function check(tokens) {
  ...
  for (const [fName, f] of Object.entries(tokens.flavors)) {
    ...
    for (const hue of tokens.variant_hues) {
      ...
      if (r < 4.5) {
        warns.push(`✗ ${fName}/${hue}-${shade} (${accent}) on ${bg}: ${r.toFixed(2)}:1`);
      }
    }
    for (const [tName, color] of Object.entries(f.syntax)) {
      const r = contrast(color, bg);
      if (r < 3) {
        warns.push(`⚠ ${fName}.syntax.${tName} (${color}) on ${bg}: ${r.toFixed(2)}:1`);
      }
    }
    ...
    for (const hue of tokens.variant_hues) {
      ...
      if (r < 4.5) {
        warns.push(`✗ ${fName}/${hue} selection text on ${sel}: ${r.toFixed(2)}:1`);
      }
    }
  }
  return warns;
...
function selfTest() {
  const cases = [
    ...
    [() => mix('#d8b4fe', '#171717', 0.25),                '#473e51', 'midnight selection (purple)'],

    // alphaOver
    [() => alphaOver('#ffffff', '#171717', 0.08),          '#2c2c2c', '8% white over midnight'],
    [() => alphaOver('#000000', '#f5f5f5', 0.05),          '#e9e9e9', '5% black over noon'],

    // contrast (just sanity-check)
    [() => Math.round(contrast('#ffffff', '#000000')),     21,        'pure black/white = 21:1'],
...
async function main() {
  ...
  console.log('✓ All (flavor, variant) selections remain readable for body text.');
}
