...
function json5ToJson(src) {
  ...
  while (i < n) {
    ...
    if (c2 === '/*') { const e = src.indexOf('*/', i + 2); if (e < 0) throw new Error('Unterminated block comment'); i = e + 2; continue; }
    if (c === '"') { out += c; i++; while (i < n) { if (src[i] === '\\') { out += src[i] + src[i+1]; i += 2; continue; } if (src[i] === '"') { out += src[i]; i++; break; } out += src[i]; i++; } continue; }
    if (c === "'") { out += '"'; i++; while (i < n) { if (src[i] === '\\') { out += src[i] + src[i+1]; i += 2; continue; } if (src[i] === "'") { out += '"'; i++; break; } if (src[i] === '"') { out += '\\"'; i++; continue; } out += src[i]; i++; } continue; }
    out += c; i++;
  }
  out = out.replace(/,(\s*[}\]])/g, '$1');
  out = out.replace(/([{,]\s*)([A-Za-z_$][A-Za-z0-9_$]*|[0-9]+)\s*:/g, (m, pre, key) => `${pre}"${key}":`);
  return out;
}
function resolveRefs(node, root) {
  if (typeof node === 'string') {
    ...
    if (typeof cur === 'string' && cur.startsWith('$')) return resolveRefs(cur, root);
    return cur;
  }
...
function rootBlock(tokens) {
  ...
  for (const [k, v] of Object.entries(tokens.shadows)) {
    const prop = k.startsWith('focus_ring') ? `--vl-${kebab(k)}` : `--vl-shadow-${kebab(k)}`;
    lines.push(`  ${prop}: ${v};`);
  }
  ...
  for (const [groupName, obj, prefix, defaultRegex, defaultName] of groups) {
    ...
    for (const [k, v] of Object.entries(obj)) {
      ...
      if (defaultRegex && defaultRegex.test(k)) {
        ...
        prop = k === 'bg' ? '--vl-bg' : `--vl-bg-${kebab(k.replace(/^bg_/, ''))}`;
      } else if (groupName === 'text') {
        prop = k === 'fg' ? '--vl-fg' : `--vl-fg-${kebab(k.replace(/^fg_/, ''))}`;
      } else {
        prop = prefix + kebab(k);
      }
...
function variantBlock(tokens) {
  ...
  lines.push(`/* Selection is derived from --vl-accent via color-mix at runtime. */\n`);
  ...
  lines.push(`  --vl-selection: color-mix(in srgb, var(--vl-accent) 25%, var(--vl-bg));`);
  lines.push(`}`);
...
async function main() {
  ...
  if (check) {
    let existing = '';
    try { existing = await readFile(cssPath, 'utf8'); } catch {}
    if (existing !== css) {
      console.error(`✗ ${relative(ROOT, cssPath)} is out of date — run \`node tools/build-css.mjs\`.`);
      process.exit(1);
    }
  ...
  console.log(`✓ wrote ${relative(ROOT, cssPath)} (${(css.length/1024).toFixed(1)} kB)`);
}
